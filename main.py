...
tf.compat.v1.flags.DEFINE_integer("d_feature", 12, "the feature length of each item in the input list")
...
tf.compat.v1.flags.DEFINE_integer("pos_embedding_mode", 1,
                                  "param used in drr_model")  # 0:use fix PE  1:use learnable PE  2:unuse PE
...
def get_model():
    model = None
    t = DrrModel(FLAGS.seq_len, FLAGS.d_feature, d_model=FLAGS.d_model, d_inner_hid=FLAGS.d_inner_hid,
...
def train():
    ...
    model.fit(train_gen, epochs=FLAGS.train_epochs, steps_per_epoch=FLAGS.train_steps_per_epoch,
              verbose=2, callbacks=callback_list, validation_data=validation_gen,
